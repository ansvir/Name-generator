...
public class GeneratorForm {
	...
	private JTextField generate_field, name_length_field, surename_length_field;
	...
	private JFrame jfrm;
	private JLabel jlb, status_line;
	...
	GeneratorForm() {
		...
		jfrm=new JFrame("NameGeneration");
		...
		jcb=new JCheckBox("Generate using certain lenghts", false);
		
		jlb=new JLabel("Enter maximum lengths of the name and surename.");
		...
		generate_button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				String name_length_str=name_length_field.getText(), surename_length_str=surename_length_field.getText();
				NameGeneration ng=new NameGeneration();
				name_length=surename_length=0;
				try {
					name_length=Integer.parseInt(name_length_str);
					surename_length=Integer.parseInt(surename_length_str);
					...
					if(surename_length<2) {
						generate_field.setText("The length of the surename should be 2 or more.");
						surename_length_field.setText("2");
						return;
					}
					
					if(jcb.isSelected()) {
						generate_field.setText(ng.generateNameAndSurename(name_length, surename_length, true));
					}
					else {
						generate_field.setText(ng.generateNameAndSurename(name_length, surename_length, false));
					}
		...
		settings_confirm.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				d_temp=(NameGeneration.SUM_OF_PROBABILTIES-sum_of_changed_prob)/(NameGeneration.PROBABILITIES_LENGTH-changed_prob.size());
				...
				double sum=0.0;
				for(int i=0;i<NameGeneration.PROBABILITIES_LENGTH;i++) {
					sum+=Double.parseDouble(letters_tf[i].getText());
				}
				
				status_line.setText(Double.toString(sum)+" Probability's settings confirmed");
			}
		});
